...
def test_supabase_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print("   成功获取 " + str(len(data)) + " 条数据")
            if data:
                print("   最新10条数据:")
                for i, item in enumerate(data[:5]):  # 只显示前5条
                    print("   " + str(i+1) + ". " + str(item))
                if len(data) > 5:
                    print("   ... 还有 " + str(len(data) - 5) + " 条数据")
        else:
            print("   获取数据失败: " + str(response.status_code))
        print()
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                print("   分类数量: " + str(len(category_list)))
                if category_list:
                    print("   分类: " + ", ".join(category_list))
        else:
            print("   统计失败: " + str(response.status_code))
        print()
